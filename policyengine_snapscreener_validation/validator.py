...
class SNAPValidator:
    ...
    def __init__(self, use_scraper: bool = False, headless: bool = True):
        ...
        self.scraper = SNAPScreenerScraper(headless=headless) if use_scraper else None
    ...
    def validate_single(
        ...
        pe_result = self.policyengine_calc.calculate(
            household, year=year, include_tanf=include_tanf, trigger_sua=trigger_sua
        )
        ...
        comparison = {
            ...
            "tanf_amount": pe_result.get("tanf_benefit", 0) / 12 if pe_result.get("tanf_benefit", 0) else 0,
    ...
    def print_comparison(self, comparison: Dict):
        ...
        print(f"  Monthly Employment Income: ${comparison['monthly_income']:,.0f}")
        print(f"  Monthly Rent: ${comparison['monthly_rent']:,.0f}")
        
        # Results
        print(f"\nBenefit Calculations:")
        print(f"  SNAP Screener:  ${comparison['screener_benefit']:,.2f}/month")
        print(f"  PolicyEngine:   ${comparison['policyengine_benefit']:,.2f}/month")
        ...
        print(f"  Max Allotment:      ${screener.get('max_allotment', 0):,.2f}")
        print(f"  Expected Contrib:   ${screener.get('expected_contribution', 0):,.2f}")
        ...
        print(f"  Expected Contrib:   ${pe.get('expected_contribution', 0):,.2f}")
        
        if pe.get('utility_allowance', 0) > 0:
            print(f"  Utility Allowance:  ${pe.get('utility_allowance', 0):,.2f}")
